...
> A client-side zero-knowledge encryption module (`zk-vault`) was previously integrated as a login gate but encrypted no data, so it was removed. It can be re-introduced later for a concrete purpose (e.g. encrypting the offline store at rest). The standalone library remains available under `zk-vault-download/`.
